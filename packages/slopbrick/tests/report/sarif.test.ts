...
function makeStaleCompleteFirstScanReport(): ProjectReport {
  ...
  const input = makeReport({
    ...
    failed: 0,
    skipped: 0,
    issues: [sampleIssue, secondIssue],
    scoreExplanation: {
      repositoryHealth: {
        value: 73,
        inputs: [
...
describe('formatSarif — additive first-scan contract', () => {
  ...
  it.each([
    ...
    const log = JSON.parse(formatSarif(input)) as {
      runs: Array<{
        tool: { driver: { properties: { firstScan: Record<string, unknown> } } };
        results: Array<{ properties: Record<string, unknown> }>;
      }>;
    };
    const firstScan = log.runs[0]!.tool.driver.properties.firstScan as {
      status: string;
      headline: unknown;
      recommendationCount: number;
    ...
    expect(firstScan.delta).not.toHaveProperty('resolvedDetails');
    expect(firstScan.delta).not.toHaveProperty('resolved');
