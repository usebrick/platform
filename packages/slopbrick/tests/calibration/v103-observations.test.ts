...
const observations = [
  { version: 'v10.3', runId: 'smoke-001', fileId: 'sbf-a', language: 'typescript', polarity: 'verified_ai', status: 'success_zero', findingsCount: 0 },
  { version: 'v10.3', runId: 'smoke-001', fileId: 'sbf-h', language: 'typescript', polarity: 'verified_human', status: 'timeout', failureCode: 'deadline' },
];
...
describe('v10.3 observation verifier', () => {
  it('accepts one terminal observation per expected ID and derived failure/coverage artifacts', () => {
    expect(verifyV103Observations({ runId: 'smoke-001', expectedFileIdsByPolarity: expected }, observations, failures, coverage)).toEqual({ ok: true });
  });
