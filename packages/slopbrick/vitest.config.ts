...
export default defineConfig({
  test: {
    globals: false,
    environment: 'node',
    // Test files launch real CLI subprocesses, and each CLI can create its own
    // scan-worker pool. Serial files prevent that nested fan-out from
    // multiplying across Vitest workers and starving process-level tests.
