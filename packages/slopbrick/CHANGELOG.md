...
- Added one optional, strict, repository-owned `mend.importRewrites` mapping to
  ...
  finding whose parser-owned module-source span, source binding, configured
  source, and already-allowed target agree can reach the shared exact-span
  preview/apply planner. Revision 80 corrects quoted-decoy/Unicode span
  selection, config-free suggestion rendering, and direct target writes.
