...
the current dirty bytes; the latest bounded serial SlopBrick run passes **313
files / 3,596 tests** (5 skipped files / 9 skipped tests); and the
...
prebuilt authority publication/recovery checkpoint is recorded below; the
symlink-safe loader, byte-backed validator, and runtime-context source-review
anchor are prior checkpoints. The final bounded package, validity, threshold,
freshness, and self-scan evidence remains in the `Live verification follow-up`
section. Older checkpoint counts remain append-only history and are not current
release evidence.
...
`tool-authority:receipt`, `evidence:verify`, `source:census`, the
