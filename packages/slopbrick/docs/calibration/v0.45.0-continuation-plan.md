...
files / 3,544 tests** (5 skipped files / 9 skipped tests); and the
...
symlink-safe prebuilt authority-graph loader checkpoint is recorded below; the
byte-backed validator and runtime-context source-review anchor are prior
checkpoints. The final bounded package, validity, threshold, freshness, and
self-scan evidence remains in the `Live verification follow-up` section. Older
checkpoint counts remain append-only history and are not current release
evidence.
