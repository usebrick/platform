...
**Current-state pointer (2026-07-14):** the latest bounded Task 2B
runtime-context source-review anchor is recorded in the checkpoint below; the
final bounded package, validity, threshold, freshness, and self-scan evidence
remains in the `Live verification follow-up` section. Older checkpoint counts
remain append-only history and are not current release evidence.
