...
- [`v10.3-release-asset-materialization-plan.md`](./v10.3-release-asset-materialization-plan.md),
  SHA-256 `3c115b75b63544c8fa281fb32532be25ed5d0a71b0afad7571163829668e93c5`;
...
- independent full-plan verdict
  ...
  `.superpowers/sdd/v103-release-task3-cache-plan-rereview.md`.
...
2. release-materialization Tasks 1-3 are complete at `ca72d0b15`,
   ...
   specification/code-quality/security approval; continue Tasks 4-6 test-first and
   independently review every slice—this tranche is the sole
   owner of the additive release-archive, safe-ZIP, receipt, checkout-map,
   identity, and resolver contracts
3. execute admission Tasks 0-3B as vertical red-green slices: tool/evidence
...
4. execute admission Tasks 4-8 on
...
5. before admission Task 9B's final commit/tarball phase, close the existing
...
6. execute admission Task 9A at its single-writer Core-schema point, then Task
...
7. execute release Tasks 7-8 without creating a side manifest or modifying the
...
8. after canary review/method freeze, set the full-run population to all and
...
9. review/apply any canonical verdict changes in one reversible commit, then
   finish Node policy, packed package, GitHub/npm, website, Cloudflare,
   accessibility, security, hook, and self-scan truth gates
10. reconcile every checkbox/evidence row and create a release candidate;
    push, tag, GitHub release, npm publication, and deployment still require
    explicit authorization and every applicable gate
11. only after the release program, begin Repository-Native Memory Phase 0;
    it is not a v0.44.0/v0.45.0 gate

The next implementation task is release-materialization **Task 4**, not corpus
