...
passes. The existing non-fatal Zod declaration warnings remain unchanged. The
next work is to bind this receipt/projection boundary into the explicit outer
materializer, then run one real-scale authority attempt only after source
evidence and rights/authorship inputs are materialized.
