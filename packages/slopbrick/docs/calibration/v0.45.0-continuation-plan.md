...
The fourth red-green Task 2B contract slice is committed at `593b32b50`. Core
now owns a strict `calibration-admission-pre-witness-bundle` schema and
generated type, plus a pure canonical self-hash validator. This boundary
...
TDD evidence is explicit: the initial focused run failed in **3/3** tests
because the contract export was absent; after the minimal schema/module/export
implementation, the focused contract and schema tests passed **9/9**. The
post-commit Core gate is green: codegen freshness, strict schema fixtures,
full Core tests (**27 files / 199 tests**), typecheck, and build. This is only
the witness-free artifact boundary; the full rich pre-witness composition
(tool-authority snapshot, record stream, acquisition/evidence references),
runtime context/disposition brand, transactional static-authority rebuild/
recovery, and diagnostic `census:preview` remain open. No corpus labels or
durable corpus artifacts changed.
