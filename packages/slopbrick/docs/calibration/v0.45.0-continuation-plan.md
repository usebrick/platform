...
- [ ] self-scan output passes the manual correctness/usefulness review
  - Partial evidence: rebuilt packaged v0.44.0 source self-scan completes
    328/328 with no parse failures or stderr noise; JSON exposes score
    direction and `scoreBasis` provenance. Fresh results are recorded in
  ...
  - Corrected in part 2026-07-11 at `efb069b90`: the exact Task 4B bytes first
    ...
    hook left all `.slopbrick` bytes unchanged. Keep this gate open for the
    remaining type-only-use, irrelevant-axis, suppressed-count,
    baseline-migration, and installed-binary defects.
- [ ] installed pre-commit hooks execute a deterministic reviewed SlopBrick
  binary
  - Current `slopbrick install` writes unversioned `npx slopbrick --staged`.
    That can download or run a different published build than the repository
    under review; this session reproduced old zero-file UX from the hook after
    local source had already changed. Prefer a verified local
    `npx --no-install slopbrick` path, or an exact-version fallback with an
    explicit network warning. Add install/upgrade/uninstall, offline, missing-
    binary, npm/pnpm, and shell-quoting tests before calling the prevention loop
    deterministic.
...
- [ ] production and development dependency advisory remediation
  - The 2026-07-11 Task 4 checkpoint is not green: production reports 17
    pre-existing advisories (3 low, 10 moderate, 4 high) through the private
    website's Astro/Vite/esbuild paths; the complete graph reports 20
    (4 low, 11 moderate, 4 high, 1 critical) through Astro/Vite/esbuild, YAML,
    and Vitest. The exact ZIP dependency delta adds none, but release Task 6
    package proof is blocked until the separate remediation/reachability tranche
    is reviewed.
  - Execute the evidence-backed remediation as isolated commits after Task 5:
    first
    move all workspace Vitest and coverage packages to exact `3.2.7` and keep
    that test graph on fixed Vite `6.4.3`; then migrate the private website from
    Astro 4 through the v5/v6/v7 guides to exact Astro `7.0.7` on Node
    `>=22.12`, refreshing the YAML language-tool chain to `yaml@2.8.3`; finally
    trial only the `tsup@8.5.1 > esbuild@0.28.1` edge as a scoped override.
    If that unsupported-edge trial breaks build/pack proof, remove it and record
    one low, dev-server-only exception with an owner and expiry rather than
    applying a global override. Both moderate-level audits must otherwise exit
    zero before Task 6.
...
corepack pnpm --filter slopbrick cal:scan -- --run "$RUN_DIRECTORY" --resume
...
corepack pnpm --filter slopbrick exec -- slopbrick scan --workspace packages/slopbrick/src --no-telemetry
```
...
- [`v10.3-release-asset-materialization-plan.md`](./v10.3-release-asset-materialization-plan.md),
  SHA-256 `08d30d9dca5b177764c5c631d98d570d75b2702a556438eaf8c10dadf292c26f`;
- [`v10.3-corpus-source-admission-plan.md`](./v10.3-corpus-source-admission-plan.md),
  SHA-256 `233ede7262dd408318230f32ab4cc5d2103caf8b5662ae4fb228088531f70e75`;
...
2. release-materialization Tasks 1-3 are complete at `ca72d0b15`,
   `704abef0e`, and `14998b539` with exact Core/SlopBrick gates and independent
   specification/code-quality/security approval; Task 4A is complete at
   `521c0e888` with 227/227 focused tests and independent review recorded in
   `.superpowers/sdd/v103-release-task4a-review.md`; Task 4B is complete at
   `6ce6259da` with 301/301 combined focused tests and independent review at
   `.superpowers/sdd/v103-release-task4b-review.md`; continue corrected Task 4
   test-first with **4C** extraction, durable publication, reuse, and cleanup;
   then continue Task 5—this tranche is the sole
   owner of the additive release-archive, safe-ZIP, receipt, checkout-map,
   identity, and resolver contracts
3. before release Task 6 can freeze packed materializer/consumer evidence,
   resolve and independently review the existing production/development
   dependency advisories; the narrower finding that Task 4's ZIP delta adds no
   advisory does not make the workspace audit green; then execute and review
   release Task 6 against the remediated graph
...
The next implementation task is release-materialization **Task 4C**, not corpus
promotion, model training, signal changes, marketing, or release. A plan defect
found during implementation reopens the relevant plan/hash review; it never
authorizes weakening provenance or readiness rules.
