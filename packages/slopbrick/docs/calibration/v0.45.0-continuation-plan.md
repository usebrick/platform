...
1. retain the completed Gate 0 containment, v0.44.0 trust-restoration scope,
   and no-ML decision; do not repeat or reopen it without new evidence
2. close SCORE-01/02 with one public score vocabulary, invariant matrix, and
   whole-project CLI/worker/MCP agreement
3. finish v10.3 run initialization, post-scan verification, denominator-aware
   metrics, machine JSON, generated report, and two-run deterministic smoke
4. extend the corpus source contract for immutable release assets and generated
   bundles before promoting any material that is not a Git-tree path; execute
   [`v10.3-release-asset-materialization-plan.md`](./v10.3-release-asset-materialization-plan.md)
   with test-first schema, identity, acquisition, extraction, resolver, packed-
   consumer, and independent-review gates
5. complete the reviewed corpus manifest, smoke, canary, method freeze, and full
   v10.3 passes; keep unproven or near-duplicate pairs quarantined
6. review/apply any canonical verdict changes in one reversible commit
7. apply the Node 22/24 policy and finish package, GitHub/npm, website,
   Cloudflare, accessibility, security, and self-scan truth
8. reconcile every checkbox and evidence row, then produce the appropriate
   release candidate and publish only after every applicable gate closes
9. after the release program is complete, begin the Repository-Native Memory
   Phase 0 research in Section 21; it is not a v0.44.0/v0.45.0 release gate

The next implementation task is **not training, marketing rebranding, or
release**. It is the remaining score/post-scan/source-contract work that every
trustworthy calibration depends on.
