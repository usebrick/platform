...
- [ ] report requested, analyzed, zero-finding, excluded, parse-failed, timed-out, and crashed file counts
  - Partial implementation evidence: `caf41e1e3` and `635838c14` add optional
    machine `scanAccounting` with selected, analyzed, zero-finding,
    incremental-cache, parse, timeout, crash, and internal terminal outcomes,
    while preserving the legacy fields. Pool and CLI contracts prove exhaustive
    accounting (37 focused tests). `excluded` remains intentionally absent
    because discovery does not yet retain reason-coded pre-selection candidates.
