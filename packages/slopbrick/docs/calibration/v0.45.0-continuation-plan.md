...
- [x] **SCORE-04 — Fix directionality once.**
  ...
  - Evidence: `1daa779bc` inverts raw aiSlopScore for repository health and
    ...
    score/maintenance/MCP/report tests pass (48/48).
...
- [ ] **SCORE-05 — Use a real exposure denominator.**
  ...
  - Invariant evidence: `622814466` deduplicates fired rule IDs before LR
    ...
    SCORE-02, SCORE-04, and cross-renderer denominator metadata remain open.
  - `9e1bf8d97` carries the analysed-file denominator as `scoreBasis` through
    JSON, pretty, Markdown, SARIF, MCP, and health outputs; metadata contract
    review remains open.
...
- [ ] **CLI-02 — Make every advertised flag observable.**
  - Partial evidence: `6fbfa2b11`/`f45ea9af5` correct the brief-report footer
    ...
    `--no-color`; a full audit of every remaining advertised flag/alias
    remains open.
  - Remaining: exercise every spelling/alias in subprocesses (including
    `--no-telemetry`, output-file forms, and CI/watch forwarding), then either
    mark the contract complete or document any intentionally no-op compatibility
    flags. Do not remove a published flag without a migration note.

- [x] **CLI-03 — Forward all filters to workers.**
  - Evidence: `043a0dc35` threads rule/include/exclude filters through worker
    data and adds a >3-file regression; scan-completion 12/12, typecheck, and
    build pass.
...
- [ ] **CLI-04 — Command smoke tests.**
  - Focused evidence: built `dist` binary smoke-tested JSON scan, `--security-only`,
    ...
    documented exit-2/config-validation path, with scan-completion 14/14. MCP
    startup and source/CJS/ESM/tarball parity remain open.
...
server/pattern/suggest tests pass 53/53. Full CLI↔MCP golden parity remains
intentionally open.
...
- [ ] compare MCP and CLI results under the same resolved config/constitution
  - Partial evidence: `060714730` loads and injects the resolved workspace
    config at MCP startup; server/pattern tests pass 41/41. Full golden parity
    remains open.
...
- [ ] **ENGINE-03 — Make public API documentation executable.**
  ...
  - Partial evidence: `eb8d4f105` removes stale engine README symbols and points
    the API contract at `packages/engine/tests/api.test.ts`; API/parser tests and
    TypeScript pass. Generated signature docs and executable examples remain.
