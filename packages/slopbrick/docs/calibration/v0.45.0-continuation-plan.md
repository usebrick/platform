...
The runnable CLI boundary is now measured: `source:census` rejects the empty
bundle when no real `evidence:verify` invocation intent and successful tool
receipt are present. No synthetic authority was added. The next ordered work
is a reviewed evidence-verification authority path, then generator/rights/
lineage recovery, an acquisition-bound register delta only for proven new
sources, and the deterministic census. The admission ledger stays **2/76**;
authoritative counts stay `verified_ai=0`, `verified_human=0`,
`eligible_gold=0`. Do not download another public repository until a measured
census deficit justifies it.
