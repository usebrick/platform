...
| --- | --- | --- |
| Local launcher, watch fallback, CLI recovery output, and diagnostic census | **Done locally; full gate capability-aware** | Commits `b4f92f1ca`, `4055304fc`, and `02d63bd1c`; focused CLI slice **21/21**, recursive typecheck/build green. The new preflight reports `ready` versus `environment_inconclusive` before the full suite; paired beacon boundary is **6/6**. A host-capable replay is still required for release evidence. |
...
is explicitly inconclusive because loopback listen returns `EPERM` and the
setuid bit is stripped; 64 descriptors open successfully and recursive watch
is available. CI, publish verification, and the main-branch pre-push hook now
run the contract and required preflight before the full suite. This improves
diagnosis only; it does not authorize corpus, release, publish, or deployment.
