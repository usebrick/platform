...
latest hardened clean, RAM-capped serial workspace gate passes Core **33 files
/ 241 tests**, Website **11 files / 47 tests**, Engine **5 files / 59 tests**,
and SlopBrick **333 files / 3,749 tests** (5 skipped files / 9 skipped tests).
Recursive build, post-build typecheck, and test are green with only the known
non-fatal Zod declaration warnings. The clean packed artifact is now rebound
to the hardened commit `a7d3e0c18`; real static/witness authority, corpus
...
evidence. The latest clean package receipt is the dated rebind at the end of
this plan: commit `a7d3e0c18`, tarball SHA-256
`f8ec4417e0fd38e15d6ffbca852b6f0c44ad5bcb413f6a36ab976f8902a06ef7`, with
matching Node 22/24 consumer receipts.
...
| --- | --- | --- |
| Local launcher, watch fallback, CLI recovery output, and diagnostic census | **Done locally; clean host-capable full gate green** | Hardened package boundary is committed at `a7d3e0c18` and documented at `18d7383d0`; focused CLI slice **21/21**, clean recursive build → typecheck → test gate green. The capability preflight reports `ready` on the unsandboxed host; the latest hardened serial SlopBrick replay is green: **333 files / 3,749 tests** passed with **5 files / 9 tests skipped**. Core is **33/241**, Website **11/47**, and Engine **5/59**; Website Playwright/axe is **16/16**. |
...
The final local package boundary is now rebound to commit
`a7d3e0c18dd8b10a4f70094ad2b3b634c92ae7ab`, which contains the reviewed
cross-language unsupported-normalizer hardening. A disposable detached
checkout built `slopbrick@0.44.0`, passed the prepack guard, and produced
`slopbrick-0.44.0.tgz` (**2,464,850 bytes**, SHA-256
`f8ec4417e0fd38e15d6ffbca852b6f0c44ad5bcb413f6a36ab976f8902a06ef7`). The
builder stayed clean before and after packing: `dirty=false`, zero status
entries, status SHA-256
`e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855`.

The same external tarball passed the packed-consumer contract **9/9** on Node
**22.22.3** and **9/9** on Node **24.15.0**. The paired-receipt assertion also
passed; both receipts bind the same tarball, commit, clean status, ESM/CJS
exports, prefer-offline consumer installation, CLI resolution, and
materialize/select/verify flow. The receipt files are preserved in the local
ignored `.superpowers/sdd/task-6-receipts/` directory. The first sandbox run
was correctly classified as a pnpm store-symlink `EPERM`; host-capable reruns
passed without changing repository or corpus bytes.

The same detached checkout then passed recursive typecheck and the serial
build → test gate: Core **33/241**, Website **11/47**, Engine **5/59**, and
SlopBrick **333/3,749**, with the expected SlopBrick **5 skipped files / 9
skipped tests**. Only the existing non-fatal Zod declaration warnings remain.
This closes the local commit-bound package/workspace prerequisite. It does not
materialize missing static-ledger or witness authority, admit corpus units,
authorize calibration, cut a tag, publish to npm, deploy the website, or
change any remote state.

## Current external authority recheck — 2026-07-16

The built `census:preview` command was rerun read-only after the hardened
artifact rebind against `/Users/cheng/corpus-expansion/v10.3`, using the
indexed `admission-context-v1` profile, invocation intent
`ea082ec3c7de8c02c89031cfc93afcbc42fc9b96a1481dce4bb94aa6dbe3cfbf`, and the
genesis register/review pair. It returned `ok=true`, `ready=false`, and
`authorityEligible=false`: **329/329** registered/reviewed sources,
**452,382** additive/quarantined units, and **0** eligible units. The only
blockers remain `static_authority_unavailable` and
`witness_authority_unavailable`.
...
`review/admission/static-ledgers/`, and `review/admission/witnesses/` trees are
still absent. The JSON output was captured read-only at
`/private/tmp/v103-census-current-20260716.json` (SHA-256
`95ed5b57a4ccab0bd3eb21aef150088a34632b60f843f0200c204dc65e50ffda`). No
source bytes, labels, manifest, authority, release, remote, publish, or
deployment state changed. The next action remains owner-supplied immutable
authority evidence; no additional repository pull is justified.

## Product self-scan UX and correctness review — 2026-07-16

The package-local CLI was run directly from `packages/slopbrick` so a stale
packed consumer could not substitute another binary:

```text
node ./bin/slopbrick.js scan --workspace . --threads 1 --no-telemetry --format json
```

The scan completed with `completionStatus=complete` and `scoreValidity=valid`.
All **248/248** selected files were analyzed; parse, timeout, crash, and
internal-failure counts were all zero. The JSON artifact is preserved outside
the repository at `/private/tmp/slopbrick-selfscan-current-direct.json`
(SHA-256
`b18340d58362e8c9890df8a04bf4ca6491b00cf6c75ad6d6c0428ce0d58f71f4`).

The result is a **policy-gate failure, not a scan failure**: `meanSlop` is
**17.9540** against the configured limit **15**, while `p90Slop=1` and
`peakScore=8.2` remain below their limits. The report contains zero high,
11 medium, and 159 low issues, with 448 suppressed issues. The main score
components are AI **11.5149** and logic **3.1786**; visual is **28.1839**,
engineering hygiene **99.4702**, and repository health **92.6595**.

The top offenders are admission/authority implementation files, which is
expected to be diagnostic feedback on calibration infrastructure rather than
evidence that the corpus or detector rules should be changed. The CLI's
`failed=0` field is the scan-accounting count for failed files; the separate
`failedThresholds=["meanSlop"]` field is the policy result. This distinction
is already represented in the report contract and should be made more obvious
in future human-facing output, but it is not a correctness blocker for this
release slice.

The run also warned that the existing baseline has a configuration-hash
mismatch and was ignored. Regenerating a baseline is a separate, controlled
diagnostic action; it must not be done in the active checkout or used to mask
the policy result. No source, rule, corpus, authority, release, or remote
state changed. The next release-critical action remains receipt-backed,
owner-supplied v10.3 authority materialization.

## Exact finite-sample KS engine slice — 2026-07-16

The reusable `@usebrick/engine` KS implementation now uses the exact pooled
label distribution for integer samples with `n + m <= 40`, via a bounded
dynamic-programming path count; larger samples retain the Hodges asymptotic
fallback. This closes the documented small-sample gap without changing any
SlopBrick detector rule, threshold, corpus label, or calibration result.

TDD evidence is red-to-green: the new finite-sample test first observed the
old asymptotic value `0.329104789...` instead of the exact `90/C(10,5) =
0.357142857...`; after the implementation, Engine passes **5 files / 60
tests**, SlopBrick's KS contract passes **21/21**, both package typechecks pass,
and the rebuilt Engine succeeds. Independent SciPy exact references agree for
`(n,m,D)=(5,5,0.6)`, `(3,7,5/7)`, and `(4,4,1)`.

This is a local statistical-engine improvement only. The previous clean
release artifact and recursive gate remain bound to commit `a7d3e0c18`; the
new code must be committed, then the clean package/consumer artifact and
recursive gates must be rebound before any release decision. The v10.3
authority and no-release gates are unchanged.

## External source-candidate review — 2026-07-16

Because the live blocker is authority rather than file count, a bounded
read-only market/data review was performed before considering another pull.
The candidates below are leads for an owner-approved evidence round, not
admitted corpus sources:

- [SWE-chat](https://www.swe-chat.com/) publishes a Hugging Face dataset with
  **205 repositories, 5,851 sessions, 14,459 commits, and 13,406 checkpoints**.
  Its schema exposes commit diffs, per-file `agent_only`/`human_only`/`mixed`
  attribution, session metadata, and repository URLs. This is the strongest
  candidate for row-level AI/human provenance because attribution is tied to
  recorded agent checkpoints rather than inferred from a detector. Access
  requires accepting the dataset conditions and sharing contact information;
  the dataset is marked ODC-BY, while the underlying public repository code
  remains subject to each repository's license. It therefore still needs an
  immutable release/commit snapshot, exact byte and path manifest, rights and
  PII audit, source-owner/lineage review, and independent admission decisions.
- [Agentic AI Coding Tool Configurations](https://se-uhd.de/ai-config-dataset/)
  provides public-repository configuration artifacts and AI-co-authored commit
  metadata, is offered under CC BY 4.0, and explicitly says source-repository
  content retains its original licenses. It is useful for repository/context
  controls and telemetry calibration, but commit-trailer/metadata detection is
  not by itself authoritative row-level authorship evidence.
- [Dryad's global AI-coding dataset](https://datadryad.org/dataset/doi%3A10.5061/dryad.3r2280gv0)
  contains classifier-labelled Python functions and large GitHub-derived
  aggregates. Those labels are model predictions, so this is a diagnostic
  comparison source, not a verified positive/negative admission source.

The already-materialized Mendeley and OSS-forge sources remain quarantine-only
under the existing audit. No candidate was downloaded, copied, relabelled, or
added to the 329-entry register in this review. Do not pull a third source
until the owner selects one and supplies the source snapshot, rights/lineage
evidence, and the complete generation/approval/static-ledger/witness bundle.

## Exact-KS clean artifact and workspace rebind — 2026-07-16

The exact finite-sample KS change is now verified from a disposable detached
checkout of commit `e29c203baca5b806b3bf7b5708ec913cdd6a150c`. The clean order
was build → typecheck → serial test. Core passed **33 files / 241 tests**;
Website **11 / 47**; Engine **5 / 60**; and SlopBrick **333 / 3,749**, with
the expected **5 skipped files / 9 skipped tests**. The only build notices
were the existing non-fatal Zod declaration warnings. The Kotlin native parser
was verified on both Node **22.22.3** and **24.15.0** after restoring the
declared package-local grammar build; no source or lockfile change was made.

The clean package passed the prepack guard and produced
`slopbrick-0.44.0.tgz`, **2,465,764 bytes**, SHA-256
`83ed688db8199193e1df9b85f488a60fb455d6f15a548547a5c726470c2969b0`. Its
builder identity was clean before and after packing: `dirty=false`, zero
status entries, status SHA-256
`e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855`.
The same external tarball passed the packed-consumer contract **9/9** on both
Node 22 and Node 24, with paired receipts at
`.superpowers/sdd/task-6-receipts/` (receipt SHAs
...
The first pack attempt hit the host's root-owned `~/.npm` cache; rerunning
with an isolated `/tmp` cache passed and did not alter repository bytes. This
closes the local package/workspace prerequisite for the current code commit.
It does not create a release, tag, publish, deploy, or calibration authority.
The v10.3 census remains `ready=false`, `authorityEligible=false`, with
`static_authority_unavailable` and `witness_authority_unavailable`; no corpus
unit may be promoted until the owner-supplied authority bundle arrives.

## Policy-gate-first-line UX correction — 2026-07-16

The human `formatPretty` verdict now states the completed policy result in its
first verdict line. A scan that is numerically in a low band but fails the
configured gate no longer reads as an unexplained success before the later
threshold table: it now says, for example, `Policy gate fails (AI Slop Score
18.0 > 15).` Passing results use the symmetric `Policy gate passes (... ≤ ...)`
wording. JSON, scores, thresholds, rules, exit codes, and corpus semantics are
unchanged.

TDD evidence is red-to-green: the new regression first failed because the
first line omitted the policy result, then passed after the minimal renderer
change. The focused UX file passes **60/60**; the renderer/CLI/score contract
set passes **25/25**; SlopBrick typecheck and build pass. A direct package-local
self-scan then analyzed **248/248** files with zero parse, timeout, crash, or
internal failures and produced the first-line failure wording while retaining
the expected diagnostic policy result `meanSlop=17.9540 > 15`.

The serial full-suite attempt passed **329/338 files (3,743/3,759 tests)** in
the restricted runner; its seven failures were environment-sensitive (five
localhost `EPERM` beacon tests, one setuid observation, and one packed-consumer
subprocess). A host-capable rerun of all four affected contract files passed
**169/169**. This is sufficient evidence for the UX slice, but the clean
commit-bound package/consumer artifact must be rebuilt after this pending code
commit. The v10.3 authority blocker and no-release gates are unchanged.

## UX commit-bound artifact and recursive gate rebind — 2026-07-16

The UX correction is committed as `d5e1e4e3790fa1375de9fcd0e9e25a95f7b9054b`.
A disposable clean checkout passed the build and recursive typecheck gates;
the serial workspace tests passed Core **33/241**, Website **11/47**, and
Engine **5/60**. SlopBrick passed **329/338 files** in the restricted runner
with the same seven host-bound failures documented above; host-capable reruns
of the four affected files passed **169/169**. The focused UX regression
remains **60/60**.

The clean prepack guard produced `slopbrick-0.44.0.tgz` (**2,466,120 bytes**,
SHA-256
`bfa3e9b104590e7af3ffa0ac15bf126f8f7eb5192bea5e94f99076d1f7bd8510`). Builder
identity stayed clean before and after packing (`dirty=false`, zero entries,
status SHA-256
`e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855`). The
exact external tarball passed the packed-consumer contract **9/9** under Node
**22.22.3** and **9/9** under Node **24.15.0** with pair assertion. New
receipt SHAs are `52fe85ee52644c070e631777175e8e322cb1284479da976ce62eb6ddb9ed43f4`
and `d8910739e8d795389bfadcdc0f9ff6043a6d5bf8469f3aaeb85540f04b740b10`.

This closes the local commit-bound package/consumer prerequisite for the UX
commit. It does not create a release, tag, publish, deploy, calibrate, admit
corpus units, or resolve the missing v10.3 static-ledger/witness authority.

## Host-capable full SlopBrick gate — 2026-07-17

The complete SlopBrick suite was replayed from the clean `d5e1e4e37` checkout
with one worker and host-capable permissions. It passed **333 test files** and
**3,750 tests**, with the expected **5 skipped files / 9 skipped tests**. The
disposable root dependency symlink was needed only for source-side zod
resolution during the replay and was removed afterward; the clean checkout
returned to zero status entries at the same commit. This closes the full local
test gate for the UX commit. It does not change the already-bound artifact or
the v10.3 authority/release blockers.

## Active roadmap ledger refresh — 2026-07-17

This refresh supersedes older ledger rows above; historical checkpoint counts
remain provenance, not current status.

| Step | Current status | Required next evidence |
| --- | --- | --- |
| Local scanner, CLI, website, package, consumer, and full test gates | **Complete locally** | Code commit `d5e1e4e37`; clean artifact SHA `bfa3e9b104590e7af3ffa0ac15bf126f8f7eb5192bea5e94f99076d1f7bd8510`; host-capable SlopBrick 333 files, 3,750 passed / 9 skipped tests. |
| v10.3 authority bundle | **Waiting for owner input** | Hash-pinned source generations/approvals, rights and authorship evidence, overlap resources, static-ledger receipts, witness assignments/reviews/receipts, and tool-authority receipts. |
| Static and witness authority materialization | **Pending** | Run only after Core-schema, hash, path, provenance, real-scale-resource, and receipt verification passes. |
| Diagnostic smoke | **Pending** | Exactly 100 verified AI-positive + 100 verified human-negative units; review exact witness IDs. |
| Calibration canary | **Pending** | Exactly 5,000 verified AI-positive + 5,000 verified human-negative units after smoke approval. |
| Release, npm publish, website deploy | **Not authorized** | Rebind release evidence after canary and release gates, then obtain explicit owner authorization. |

### Owner authority bundle intake contract

The next input should be one immutable, hash-pinned bundle rooted at
`/Users/cheng/corpus-expansion/v10.3`. It must include:

1. Source generation/current/approval bytes and an exact path/hash manifest for
   every registered source, including rights, lineage, authorship protocol,
   repository/commit identity, and independent review decisions.
2. Overlap universe, records, policy, normalizer registry, shard/resource
   receipts, and the real-scale stream/resource receipt used by the static
   ledger.
3. Static-ledger generation/current pointers plus record/result/unresolved
   JSONL projections, partition/count/record-set/semantic hashes, and
   publication receipts.
4. Witness policy, deterministic search bundle, blind assignment/receipt,
   witness-review bundle, decisions, and publication completions for `smoke`
   first; repeat the same graph for `canary` only after smoke passes.
5. Indexed tool profile, invocation intent, tool receipt, executable snapshot,
   and canonical argument/input/output hashes for every publication.

The first verification command remains read-only `census:preview`. It must
return `ok=true`, `ready=true`, `authorityEligible=true`, non-zero eligible
counts, and no `static_authority_unavailable` or
`witness_authority_unavailable` blockers. Until then, all **452,382** units
remain quarantine-only and no additional repository pull is justified.

## Website browser/UX gate and copy correction — 2026-07-17

The live website was reviewed through the real Playwright browser at desktop
and 320px mobile widths. The review found two user-visible issues that the
existing axe and overflow checks did not catch: the generated corpus label
rendered as `Historical v10.1 historical` in several sections, and a later
`.hero__title-accent` rule restored `white-space: nowrap`, clipping
`repository structure` on narrow screens. The fix keeps the generated data
label machine-readable as `v10.1`, adds `historical` at presentation sites,
adds explicit spaces around interpolated counts, and allows the mobile title
accent to wrap within its column. A 320px bounding-box regression now proves
the accent stays inside the title and uses normal whitespace.

The browser/accessibility suite is green at **17/17** (critical and serious
axe checks, keyboard focus, live-terminal interactions, route status, robots,
sitemap, mobile overflow, and the new mobile title assertion). Website
typecheck is **47 files / 0 diagnostics**, unit tests are **11 files / 47
tests**, and the static build produces all **4 routes**. Manual browser
verification confirms the corrected strings (`Historical v10.1`, no duplicate
label, and spaced `119 rules, 27 categories, measured evidence`). This closes
the website UX gate locally; it does not alter SlopBrick scoring, corpus
authority, release authorization, npm publishing, or deployment state.

The next evidence-bearing step is unchanged: obtain the owner-supplied,
hash-pinned v10.3 authority bundle, rerun read-only `census:preview`, then
materialize static-ledger/witness authority before the exact 100+100 smoke.

## Fresh v10.3 census recheck — 2026-07-17

The read-only census was rerun against the current
`/Users/cheng/corpus-expansion/v10.3` tree with the indexed
`admission-context-v1` profile and invocation intent
`ea082ec3c7de8c02c89031cfc93afcbc42fc9b96a1481dce4bb94aa6dbe3cfbf`. The
canonical output is byte-identical to the prior receipt (SHA-256
`95ed5b57a4ccab0bd3eb21aef150088a34632b60f843f0200c204dc65e50ffda`):
`ok=true`, `ready=false`, `authorityEligible=false`, 329/329 registered and
reviewed sources, **452,382** additive units, **0** eligible units, and the
same blockers `static_authority_unavailable` and
`witness_authority_unavailable`. The evidence-context hash remains
`9b3f8a4adcbb82ef4d97685be51264a1feb44ebf46584697df93511dad2f3089`; the
source-register hash remains
`ce40134968cd9f490b29e695b27fb724ce8d4b8ba9a4abf26eb789cc4c4d78de`.
...
are still absent. Existing acquisition and tool-authority files do not
constitute static ledger or witness admission authority, so no labels,
manifests, corpus bytes, or release state were changed.

## Focused authority-boundary replay — 2026-07-17

The local handoff machinery was replayed serially across six bounded suites:
static-ledger stream, array-ledger contracts, outer authority materializer,
authority CLI boundary, rebuild adapter, and allocation preview. All **6 test
files / 56 tests** passed. The suite proves byte binding, stream partition and
semantic hashes, overlap joins, explicit selector requirements, no-clobber
publication/recovery, and provenance conservation on fixtures. It does not
claim live authority: the real v10.3 tree still has no static-ledger or witness
bundle, so the census remains zero eligible and no corpus state was mutated.

## Task 9A Core dependency-receipt contract — complete — 2026-07-17

The next independent roadmap slice is complete at Core commit `ef83ef3ec`.
It adds the four v10.3 dependency-receipt schemas and generated/typed
validators for release prerequisite approval, score/wire closure, run lifecycle,
and packed Node runtime. TDD evidence is recorded: the exact contract test
first failed on the missing module, then passed **16/16** after the minimal
implementation. Core schema fixtures, codegen freshness, typecheck, **34 test
files / 257 tests**, and build all pass.

This is a contract bootstrap only. It does not bump SlopBrick, create a
tarball or manifest, admit corpus units, pull repositories, publish npm, or
deploy the website. Task 9B must reuse these schemas without semantic changes
and remains gated on the owner-supplied authority bundle. The current census
is still `ready=false` / `authorityEligible=false` with 452,382 quarantine-only
units and the two static/witness blockers. Next valid action: verify that
immutable bundle, rerun `census:preview`, then proceed to Task 9B and the
ordered smoke/canary sequence only if the gate becomes ready.

## Task 9B Core manifest-contract foundation — in progress — 2026-07-17

The first bounded Task 9B slice is complete locally. Core now carries the
twelve typed, schema-indexed contracts for the prerequisite bundle/staging
set/request/lock/transaction/completion/current graph and the downstream
manifest build-receipt/generation/current/lock/transaction graph. Generated
peers are deterministic, fixed Node 22/24 and manifest leaf tuples are
strengthened in codegen, and the pure validators enforce closed artifact
kind/owner/media/schema mappings, relative paths, sorted identities,
operation/current-state consistency, fixed final projection paths, nested
handoff shape, and canonical self-hashes.

The corpus-manifest contract also now reserves optional `admissionBinding` and
per-file admission/materialization identities. The semantic verifier requires
the self-hashed binding and both identities for exact `v10.3.2`, while keeping
legacy `v10.3.0`/`v10.3.1` manifests byte-compatible and rejecting a non-null
binding on legacy methods. Core evidence for this slice is green: focused
contract tests **25/25**, corpus/schema contract tests **82/82**, schema
fixtures, schema validation, and typecheck pass.

This is not the Task 9B publisher, verifier, SlopBrick consumer integration,
release bump, packed-runtime receipt, manifest output, calibration, publish,
or deployment. The next implementation slice is the pure admission-manifest
builder/consumer boundary in SlopBrick, but it remains unable to produce a
real manifest until the owner-supplied v10.3 static-ledger/witness authority
bundle makes `census:preview` return `ready=true` and
`authorityEligible=true`.

## Task 9B SlopBrick consumer boundary — in progress — 2026-07-17

The next bounded slice is now implemented locally. SlopBrick has a shared,
read-only `openAdmissionManifestForConsumer` boundary. It accepts the exact
`{root, manifestId, manifestReference, expectedManifestSha256}` input, parses
and self-hash-checks the canonical routing reference, follows the fixed
current → generation → build-receipt → v10.3.2 manifest → immutable
prerequisite completion/request/bundle graph, validates Core contracts, checks
content-addressed prerequisite bytes, rejects orphan generation leaves and
symlink/path escapes, and rereads the current pointer immediately before
returning.

The returned value is deep-frozen and registered in a module-private
`WeakSet`; a JSON round-trip, cast, or clone cannot create the consumer brand.
The `corpus:validate`, `select`, and `cal:materialize` admission source path
now uses this boundary in its own process. Legacy flat manifests remain
restricted to `v10.3.0`/`v10.3.1`. No manifest, corpus authority, output cache,
selection, release, publish, deploy, or remote state was created.

TDD evidence is green: the new consumer fixture covers a complete graph,
reference mutation, generation mutation, orphan-leaf rejection, and private
brand behavior; the paired CLI compatibility suite remains green (**11 tests**
across the consumer/CLI pair). SlopBrick typecheck passes. The remaining Task
9B work is the pure builder, prerequisite publisher/verifier and packed
runtime/release evidence; the live corpus remains fail-closed until the
owner-supplied static-ledger and witness authority bundle makes
`census:preview` return `ready=true` and `authorityEligible=true`.

## Task 9B immutable prerequisite and ready-census gates — 2026-07-17

The next local trust boundary is implemented on branch `codex/v0.45-recovery`.
`admission-manifest-prerequisites.ts` reopens one caller-supplied historical
six-field reference, canonicalizes and deep-freezes the bundle/completion/
request graph, verifies every content-addressed artifact byte, checks staging
metadata and source-root bindings, validates release/score/run/runtime receipt
semantics, rejects symlink/path escapes, and registers only the exact result in
a private `WeakSet`. A JSON clone or mutated reference cannot mint the
prerequisite brand. The focused fixture also proves the artifact-byte mutation
failure path.

`admission-ready-census.ts` adds the private `VerifiedReadyAdmissionCensusV1`
boundary. It requires the branded admission context, rebuilds the census from
the exact search/review inputs, verifies the selected gate's search receipt,
witness, approved review bundle, publication completion hash/path, and exact
rebuild bytes, then brands only the deep-frozen result. A plain schema-valid
census, cast, wrong gate, unapproved review, non-hash-addressed publication,
or serialized clone is rejected. This is the pure binding slice; filesystem
reopening of the witness publication graph remains part of the next verifier
slice.

Evidence: the paired prerequisite/ready-census/consumer/CLI suite is
**5 files / 17 tests passed**; SlopBrick typecheck and `git diff --check` pass.
The live v10.3 corpus is unchanged: `census:preview` remains
`ready=false`/`authorityEligible=false` with 0 eligible units and the same
static/witness authority blockers. No manifest, labels, corpus bytes, release,
publish, deploy, or remote state changed.

### Updated execution order

1. Complete the filesystem witness-publication reopener and bind its exact
   completion/reference bytes into the ready-census verifier.
2. Add the pure manifest builder that accepts only both private brands and
   derives every file/source identity from the verified context and witness.
3. Rebind the packed Node 22/24 receipts and manifest build receipt to the
   verified behavior/census/prerequisite hashes, then run the packed consumer
   matrix.
4. Only after owner authority arrives, rerun read-only `census:preview`, then
   the ordered 100+100 smoke and 5,000+5,000 canary gates.

## Task 9B filesystem witness reopener checkpoint — 2026-07-17

The first filesystem part of the next boundary is now implemented in
`admission-witness-reopen.ts`. It reopens one hash-addressed search or review
publication below the fixed witness topology, rejects symlink/path escapes and
oversized or non-canonical JSON, validates the Core bundle and completion, and
brands only the exact deep-frozen graph in a private `WeakSet`. The ready-census
gate now requires those private publication brands and exact completion/path
joins. The reopener is routing and byte-integrity evidence; it does not yet
resolve the indexed tool-authority receipt chain or replace the domain-level
witness-review cross-validator.

Current evidence is **6 files / 19 tests passed**, SlopBrick typecheck passed,
package build passed, and `git diff --check` passed. The build emitted existing
Zod declaration-export warnings but exited successfully. The live corpus is
unchanged at `ready=false` / `authorityEligible=false`, with 0 eligible units
and the static/witness authority blockers. No manifest, corpus bytes, labels,
release, publish, deploy, or remote state changed.

The next bounded slice is to bind the publication completion's indexed tool
authority receipts and domain-level search/review reconstruction, then build
the pure manifest only from the prerequisite and ready-census private brands.

## Task 9B witness authority join checkpoint — 2026-07-17

The next trust edge is now implemented in
`admission-witness-authority.ts`. A reopened publication is not sufficient by
itself: the new boundary resolves the completion's publication receipt and
every required witness receipt from one exact current tool-authority
generation, checks profile/action/output bindings, and verifies the
domain-level search graph plus a deterministic rebuild of the review graph.
`VerifiedReadyAdmissionCensusV1` now requires both private authority joins and
binds them by object identity before it can be consumed by a manifest builder.

Evidence: focused authority/ready/reopener tests **4 files / 9 tests passed**;
SlopBrick typecheck passed; the prior six-file consumer/CLI gate remains green.
This is still fixture evidence, not live authority. The real v10.3 census is
unchanged at 0 eligible units and remains blocked by absent static/witness
authority trees. No manifest, corpus, release, publish, deploy, or remote
state changed.

Next: implement the pure manifest builder, accepting only the private
prerequisite and ready-census brands and deriving all output identities from
their verified graphs.

## Task 9B pure manifest-builder checkpoint — 2026-07-17

The pure `buildCorpusManifestFromAdmission` boundary is now implemented and
exported. It accepts only the private ready-census and immutable-prerequisite
brands, projects exactly the reviewed witness units, binds repository/file
identity to the verified lineage and materialization receipts, maps only
immutable HTTPS evidence into the Core evidence union, and emits deterministic
`methodVersion: v10.3.2` output with the complete self-hashed admission binding.
It performs no corpus-folder discovery, checkout reads, network access, or
candidate-code execution. Git and release-archive materializations are
handled explicitly; record containers, unsupported rights, missing evidence,
lineage disagreement, witness mutations, and conflicting repository metadata
fail before output.

Local proof is **6 files / 16 tests passed** for the builder, witness authority,
ready census, publication reopener, and packed manifest-consumer boundaries;
SlopBrick typecheck, package build, and `git diff --check` pass. The live v10.3
census remains unchanged at `ready=false` / `authorityEligible=false` with 0
eligible units because the owner-supplied static-ledger and witness-authority
trees are absent. No manifest, corpus bytes, labels, release, publish, deploy,
or remote state changed.

Next: bind the builder's behavior and Node 22/24 packed-runtime receipt set to
the immutable prerequisite graph and add the packed consumer matrix before any
owner-authority smoke/canary attempt.

## Task 9B packed-runtime contract hardening — 2026-07-17

The release boundary now enforces the plan's single-tarball invariant: Node 22
and Node 24 must reference one content-addressed package-tarball artifact, and
their receipt artifact IDs must remain distinct. The Core semantic validator,
the immutable SlopBrick prerequisite verifier, and the fixture contract now
reject a split tarball pair or a duplicated runtime receipt ID.

The serial `scripts/test-packed-runtime-matrix.mjs` gate is also present and
exposed as `slopbrick test:pack-matrix`. It packs once, extracts the frozen
`package/dist/calibration/v103/admission.cjs` builder member, runs the existing
offline packed-consumer contract under Node 22 and Node 24 one at a time, and
emits only canonical Core `v10.3-packed-runtime-receipt-v1` JSON after both
results agree on commit, package, tarball, and builder hashes. It requires
explicit expected commit/behavior/reviewer inputs, strict offline mode, and a
no-clobber output directory; it fails closed while the package is not yet
`0.45.0` or the scoped checkout is dirty. No receipt or tarball was emitted in
this checkpoint. Focused proof: Core manifest contracts **26/26**, SlopBrick
prerequisite/builder tests **5/5**, both package typechecks, script syntax, and
diff checks pass.

Next: perform the final `0.44.0` -> `0.45.0` implementation freeze, build the
package, run this matrix on the exact committed artifact, and only then stage
the owner-reviewed prerequisite bundle.

## Task 9B frozen-candidate packed diagnostic — 2026-07-17

The local candidate metadata is now frozen at `slopbrick@0.45.0` and the
diagnostic matrix ran against one packed tarball under Node 22 and Node 24.
Both runtimes passed the existing strict-offline packed-consumer contract
(9/9 tests each) and agreed on the package and builder identities:

- tarball SHA-256:
  `1e663b7e2aa7ca8108144637bcfbfd4827f6c637a9ad6e482c2005680a1cd964`
- `admission.cjs` builder behavior SHA-256:
  `1a70c53dc0f950594a7c5c283423be58a233af3e93c5bb3052c94510f1c54a96`
- observed runtimes: Node `v22.22.3` and `v24.15.0` on macOS

This was `--diagnostic-only`: it intentionally wrote no canonical runtime
receipts because independent reviewer identities and an owner-approved
prerequisite bundle have not been supplied. The local packed contract is
therefore green, but the admission/release gate remains open. The live v10.3
census is still `ready=false` / `authorityEligible=false` with 0 eligible
units. No manifest, corpus labels, release, publish, deploy, or remote state
changed.

## Task 9B latest candidate diagnostic — 2026-07-17

After the shared-tarball fixture correction and workspace build, the exact
committed candidate `0f763c6441f1a85643e60b0b201288f35c696c03` passed the
serial packed-consumer matrix on Node `v22.22.3` and `v24.15.0` (9/9 tests per
runtime). The current packed tarball SHA-256 is
`0d05f901e03d6e55bb3a9d7add531694f82ef9d211f245636cf539f3af35e72f`; the
builder behavior SHA-256 remains
`1a70c53dc0f950594a7c5c283423be58a233af3e93c5bb3052c94510f1c54a96`.

This is still diagnostic-only: no canonical receipts were written because
independent reviewer IDs and the owner-approved v10.3 authority bundle are
absent. The live census remains 0 eligible and authority-ineligible. No
manifest, labels, release, publish, deploy, or remote state changed.

The same diagnostic was re-bound to the documentation checkpoint
`5b09f945bca30414606c9349745058d86b46389e`; the tarball and builder hashes
were unchanged. The documentation-only follow-up does not alter package bytes
or create release evidence.

## Task 9B host-capable quality-gate checkpoint — 2026-07-17

The host-capable local quality gate is complete:

- workspace typechecks: Core, Engine, SlopBrick, and Website all passed;
- Core tests: **35 files / 285 tests passed**;
- SlopBrick tests: **340 files / 3,768 tests passed**, with 5 intentionally
  skipped files / 9 skipped tests;
- `pnpm -r build`: Core, Engine, SlopBrick, and the four-page Astro site passed.

The first sandboxed full-suite attempt reported network/special-bit/offline
install restrictions; the same affected suites passed with host permissions.
That distinction is recorded as environment evidence, not a product failure.
Only the website's generated candidate metadata changed during the build and
was committed separately. No release, publish, deploy, or remote mutation ran.

## Task 9B self-scan disposition checkpoint — 2026-07-17

The package-local release-checklist self-scan completed with deterministic
coverage but a policy failure: **254/254 files analyzed**, 0 parse/timeout/
crash/internal failures, **176 active findings**, and 480 default-off audit
findings. The AI-slop score is **18.3/100** against `meanSlop: 15`; Engineering
Hygiene is 99.5, Security is 100, and Repository Health is 92.5. The active
burden is primarily 165 low-severity `ai/compression-profile` findings in the
repetitive calibration/protocol code, plus 11 vocabulary anomalies.

The existing baseline was ignored because its config hash is stale. No new
baseline, rule suppression, threshold change, or source edit was made. This
output is useful evidence of a calibration/self-scan UX mismatch, not evidence
of AI authorship or a security defect. Before canonical release receipts, the
next review must decide whether to add an evidence-backed protocol-code
exception, a repository-role-aware score, or a calibrated threshold treatment.

## Task 9B baseline-recalibration UX hardening — 2026-07-17

The stale-baseline guidance is now fail-safe in commit `4622177ae`. Scan,
doctor, and cache-load paths no longer tell a user to recalibrate immediately
after an invalid baseline is ignored. They now say to review the current score
first, then run `slopbrick scan --baseline` only to create a new baseline. This
preserves the existing fail-open scan behavior while reducing the chance that a
stale configuration silently becomes a new policy floor.

Focused regression proof is **4 files / 207 tests passed** (`cache`, CLI
output-UX, scan completion, and CLI integration). No threshold, suppression,
score, corpus label, manifest, or release artifact changed. The self-scan's
18.3/100 policy failure therefore remains an explicit calibration review item;
the safe next decision is evidence-backed protocol-code handling, not automatic
baseline recalibration.

### Roadmap status after this checkpoint

| Area | Status | Next evidence |
| --- | --- | --- |
| Local Core/Engine/SlopBrick/Website typecheck, tests, build | **Complete locally** | Re-run only after material source changes. |
| Packed Node 22/24 consumer contract | **Diagnostic green** | Owner-approved reviewer identities and prerequisite authority bundle for canonical receipts. |
| Self-scan correctness/UX | **Disposition recorded; policy review pending** | Decide protocol-code exception, repository-role-aware scoring, or calibrated threshold from admitted evidence. |
| v10.3 corpus admission | **Blocked / quarantine-only** | `census:preview` must become `ready=true` and `authorityEligible=true`; static and witness authority paths are absent. |
| Smoke/canary calibration | **Pending** | 100 AI + 100 human smoke, then 5,000 AI + 5,000 human canary after authority intake. |
| Release, publish, deploy, remote mutation | **Not authorized** | Explicit owner authorization after all gates and receipts are valid. |
