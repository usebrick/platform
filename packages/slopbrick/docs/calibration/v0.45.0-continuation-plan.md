...
2. release-materialization Tasks 1-3 are complete at `ca72d0b15`,
   ...
   specification/code-quality/security approval; execute corrected Task 4 as
   three test-first and independently reviewed slices: **4A** shared trusted
   POSIX cache primitives with Task 3 behavior preserved, **4B** exact ZIP
   dependencies plus raw validator/CRC/receipt/reference codecs, and **4C**
   extraction, durable publication, reuse, and cleanup; then continue Task 5—
   this tranche is the sole
...
The next implementation task is release-materialization **Task 4A**, not corpus
