...
One serial diagnostic scan with explicit C/Python include globs analyzed
**200/200** files (100 per arm), with **0** parse failures or crashes, valid
completion, AI Slop Score **8.56**, and no failed thresholds. The machine
review is
...
The default-config control selected only the 100 Python files; C/C++ support is
documented but omitted from the default include globs. Add explicit backend
language discoverability/configuration to the future UX slice. This result is
diagnostic only and does not promote labels or authority.
