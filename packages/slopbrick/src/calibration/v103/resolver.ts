...
/** Returns no local path: canonical artifacts retain only normalizedPath. */
export async function resolveSelectedRecord(record:SelectedResolution,map:unknown):Promise<{normalizedPath:string;bytes:Buffer}>{
 ...
 try{const root=await realpath(e.checkoutPath), file=await realpath(resolve(root,record.normalizedPath)), rel=relative(root,file);if(rel===''||rel==='..'||rel.startsWith(`..${sep}`))throw Error();const bytes=await readFile(file);if(createHash('sha256').update(bytes).digest('hex')!==record.contentSha256)throw Error();return {normalizedPath:record.normalizedPath,bytes};}catch{throw new Error('Unable to resolve selected record');}
}
