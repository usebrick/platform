...
| Mend capability | One exact import-rewrite path locally qualified inside `slopbrick`; owner disposition pending and no separate package/public rollback surface | Repeated trusted repairs produce a stable contract that two independent consumers need and can version separately |
