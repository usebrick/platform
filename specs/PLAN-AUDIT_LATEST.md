...
external review-artifact validator remains valid. This remains a bounded Core
contract only: authority rebuild lock/transaction, byte-backed SlopBrick
context/disposition, CLI `rebuild:pre-witness`/`static-authority:recover`/
`census:preview`, witness/resource receipts, corpus admission, and release
