...
The audit also records the current normalizer boundary: the reviewed lexical
runtime receipt covers **228,456 TypeScript rows**, while **223,926** rows are
in other languages. This is an authority-coverage dependency, not a request to
manually review every file. Close it with reviewed language bindings (or an
explicit excluded partition) before publishing a complete overlap ledger.
