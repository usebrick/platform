...
- The continuation-plan hash override was recomputed on 2026-07-16 after the
  bounded scan UX review; any subsequent plan edit requires
...
tests. The full RAM-capped SlopBrick gate is **329 test files / 3,716 tests
...
The active calibration path is owner-led: one execution owner and one
integrated automated edge-case gate. Existing evidence is reused rather than
reconfirmed by parallel agents. A second reviewer is warranted only for a
...
The explicit C/Python diagnostic scan analyzed **200/200** files with **0**
parse failures or crashes, AI Slop Score **8.56**, and no failed thresholds.
Its audit is
`/Users/cheng/corpus-expansion/v10.3/review/formai-v1-gpt35-smoke-scan-review-2026-07-16.json`
with SHA-256
`335d11bc17ad4b1241bd055f699580d9356adf8ca7206beeb4a0f5e35ebe6203`.
The default-config control selected only Python; C/C++ is in the support matrix
but omitted from default include globs. This is a bounded UX/configuration
improvement, not a scanner or authority failure, and no labels changed.

### Default C/C++ discovery fix — 2026-07-16

The default discovery policy now includes `.c`, `.h`, `.cc`, `.cpp`, `.cxx`,
`.hpp`, and `.hxx`, matching the reviewed language-support matrix and the
documented default-on C/C++ hygiene rules. The rebuilt default control selected
and analyzed **200/200** bounded FormAI/HumanEval files (100 C + 100 Python),
with **0** parse/internal failures, valid completion, unchanged diagnostic
scores, and no failed thresholds. The before/after audit is
...
This is a product discoverability fix only; it does not promote labels or
change static/witness authority state.

### Default C/C++ discovery implementation and gate — 2026-07-16

The default discovery policy now includes `.c`, `.h`, `.cc`, `.cpp`, `.cxx`,
`.hpp`, and `.hxx`. Focused discovery/language-contract tests are **22/22**;
typecheck and rebuild pass; and the rebuilt default FormAI/HumanEval control
selects **200/200** files (100 C + 100 Python) with zero parse/internal
failures. The RAM-capped package-wide gate passed **329 test files / 3,717
tests**, with 5 skipped files / 9 skipped tests. This is a discoverability fix
only; the live census remains **0 represented / 0 eligible** and overlap
verification still fails closed with `overlap_current_missing`.
